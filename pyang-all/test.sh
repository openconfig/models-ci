...
if stat $PYANGBIND_RESULTSDIR; then
  ...
  pip3 install pyang
  {
    if timeout 300s bash $PYANGBIND_RESULTSDIR/script.sh $VENVDIR/bin/pyang --plugindir $PYANGBIND_PLUGIN_DIR > $PYANGBIND_RESULTSDIR/$OUTFILE_NAME 2> $PYANGBIND_RESULTSDIR/$FAILFILE_NAME; then
      # Delete fail file if it's empty and the script passed.
