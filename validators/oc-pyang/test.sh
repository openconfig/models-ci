...
if bash $RESULTSDIR/script.sh $VENVDIR/bin/pyang --msg-template='messages:{{path:"{file}" line:{line} code:"{code}" type:"{type}" level:{level} message:'"'{msg}'}}" --plugindir $OCPYANG_PLUGIN_DIR > $OUTFILE 2> $FAILFILE; then
